...
if ($employee['permissions']['admin'] != '1' && $data['data']['owner'] != $employee['id']) {
    echo "You are not permitted to duplicate this product.";
    exit;

} else {
    // Event
    $new_id = $cart->duplicate($_POST['id'], $data['data']['name']);

}
$task = $db->end_task($task_id, '1');

// Return Cell
$history     = new history($new_id, '', '', '', '', '', 'ppSD_products');
$table_format  = new table('product', 'ppSD_products');
